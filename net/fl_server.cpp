...
void FLServer::start() {
	if (SDLNet_ResolveHost(&my_ip, NULL, FL_SERVER_PORT) == -1) {
		std::cout << "Error resolving server host: " << SDLNet_GetError()
				  << std::endl;
		return;
	}
...
void FLServer::queue_heartbeats() {
	...
	for (int i = 0; i < FL_MAX_CONN; ++i) {
		if (client_conns[i].accepted &&
			client_conns[i].state == FL_CLIENT_CONNECTED) {
			if (tick - client_conns[i].last_heartbeat >=
				FL_HEARTBEAT_INTERVAL) {
				queue_heartbeat(i);
			}
		}
	}
...
void FLServer::update_player_info(float x, float y, float vx, float vy,
								  int animation) {
	player_info.x = x;
	player_info.y = y;
	player_info.vx = vx;
...
void FLServer::update_client_positions() {
	...
	if (elapsed_ms >= FL_POS_SEND_INTERVAL) {
		...
		// For every active client, send a message to all other clients with
		// their last position
		...
		for (int i = 0; i < FL_MAX_CONN; ++i) {
			if (client_conns[i].state == FL_CLIENT_CONNECTED) {
				...
				// We use the current slot for a client's connection to us, the
				// server, since the client doesn't need that slot for themself
				data[6] = i;
...
void FLServer::accept_heartbeat(IPaddress addr) {
	...
	if (slot >= 0) {
		// XXX again this isn't totally accurate since we won't process this
		// immediately...
		client_conns[slot].last_tick = SDL_GetTicks();
		client_conns[slot].ping =
			client_conns[slot].last_tick - client_conns[slot].last_heartbeat;
	}
}
...
void FLServer::ack_del_obj(IPaddress addr, Uint8 *data) {
	...
	if (slot >= 0 && client_conns[slot].player != nullptr) {
		...
		// TODO ? it would be cleaner in these situations to just have a struct
		// type for memcpy
...
void FLServer::sync_del_obj(uint16_t id) {
	...
	// It is a one-time generation of all the necessary synchronized messages
	// for the object deletion. This method assumes that we have already filled
	// in an entry in deleted_net_objects for the object.
...
void FLServer::mark_net_object_deleted(uint16_t id, int client_slot) {
	...
	if (client_slot >= 0 && it != deleted_net_objects.end()) {
		...
		if (synchronized) {
			...
			clear_del_item_synchronized_messages(
				id, 0); // 0 indicates to delete from any host
		} else {
			std::cout << " not synchronized yet for id " << id << std::endl;
		}
	}
	// Else, we received a message from a client about an item which does not
	// exist
}
...
void FLServer::accept_client_conn(IPaddress addr) {
	...
	if (avail_slot >= 0) {
		...
		std::cout << "accepted a connection in slot " << avail_slot
				  << std::endl;
...
void FLServer::queue_message(int slot, Uint8 *data, int len,
	...
	if (!synchronized) {
		...
		if (smsg->ack_type != FL_MSG_UNK) {
			...
			std::cout << "Server: ERROR tried to create sync message with "
		...
		if (client_conns[i].accepted) { // we only care about active slots
			if (client_conns[i].state == FL_CLIENT_CONNECTED) {
				if (tick - client_conns[i].last_tick >= FL_TIMEOUT) {
					client_conns[i].state = FL_CLIENT_DISCONNECTED;
					std::cout << "Server: Client " << client_conns[i].ip.host
							  << " disconnected\n";
				}
			}
