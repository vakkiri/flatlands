...
FLAnimatedObject::FLAnimatedObject(FLTexturedObjectParams &tex_params,
...
void FLAnimatedObject::set_start_repeat(unsigned int repeat,
										unsigned int animation) {
	start_repeat[animation] = repeat;
}
...
void FLAnimatedObject::set_repeats(bool repeats) { this->repeats = repeats; }

void FLAnimatedObject::set_animated_object_list_position(
...
bool FLAnimatedObject::finished() { return animation_finished; }
...
int FLAnimatedObject::animation() { return cur_animation; }

void FLAnimatedObject::set_animation_update_method(
	std::function<void(void)> meth) {
...
void remove_null_animated_objects() {
	animated_objects.erase(
		std::remove_if(animated_objects.begin(), animated_objects.end(),
...
void clear_animated_objects() { animated_objects.clear(); }
