...
FLShader::FLShader() { program_id = NO_PROGRAM; }
...
void FLShader::unbind() { glUseProgram(NO_PROGRAM); }
...
GLuint FLShader::load_shader(std::string path, GLenum shader_type) {
	...
	if (infile) {
		shader_string.assign(std::istreambuf_iterator<char>(infile),
...
GLuint FLShader::get_program() { return program_id; }

void FLShader::set_camera(glm::mat4 matrix) { camera_matrix = matrix; }

void FLShader::set_projection(glm::mat4 matrix) { projection_matrix = matrix; }

void FLShader::update_pc_matrix() {
	glm::mat4 pc_matrix = projection_matrix * camera_matrix;
	glUniformMatrix4fv(pc_matrix_location, 1, GL_FALSE,
					   glm::value_ptr(pc_matrix));
}
