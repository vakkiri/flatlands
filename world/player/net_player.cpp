...
// TODO: this should probably be based on the terminal velocities in the player
// class
...
void FLNetPlayer::set_target(float tx, float ty) {
	// Note that the player sends the EXPECTED position by the time this packet
	// is received, not the real position. Thus, it is just our job to smoothly
	...
	// In the case of huge jumps (assumed to be from respawning or teleporting)
	// we just move instantly.
	if (fdx > MAX_MOVEMENT || fdx < -MAX_MOVEMENT || fdy > MAX_MOVEMENT ||
		fdy < -MAX_MOVEMENT) {
		set_x(tx);
		set_y(ty);
	} else {
