...
FLShape::FLShape( std::vector<point> vertices ) {
	...
	// Current implementation uses a bounding box as the "size" for width and height.
	// One possible alternative would be to use a radius for a circular bounding shape.
	else {
...
std::vector<point>& FLShape::get_vertices() {
	return vertices;
}

void FLShape::translate( float x, float y ) {
	translate( point(x, y) );
}
...
void FLShape::set_pos( float x, float y ) {
	set_pos( point(x, y) );
}
