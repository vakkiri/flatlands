...
std::vector<std::vector<fl_colored_vertex>>& FLHealthbar::get_primitive_vertices() {
	float ratio = FLGame::instance().environment()->player()->health_ratio();

	primitive_vertices[0][1].pos.x = primitive_vertices[0][0].pos.x + (94 * ratio);
...
void FLHealthbar::init_vertices() {
	primitive_vertices = std::vector<std::vector<fl_colored_vertex>>(4, std::vector<fl_colored_vertex>(4));
	...
	for ( int i = 0; i < 4; ++i ) {
		primitive_vertices[0][i].c = {255.f/255.f, 194.f/255.f, 161.f/255.f, 1.f};
	}

	for ( int i = 0; i < 4; ++i ) {
		primitive_vertices[1][i].c = {230.f/255.f, 69.f/255.f, 57.f/255.f, 1.f};
	}

	for ( int i = 0; i < 4; ++i ) {
		primitive_vertices[2][i].c = {156.f/255.f, 42.f/255.f, 112.f/255.f, 1.f};
	}

	for ( int i = 0; i < 4; ++i ) {
		primitive_vertices[3][i].c = {105.f/255.f, 36.f/255.f, 100.f/255.f, 1.f};
	}
}
