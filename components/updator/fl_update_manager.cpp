...
std::vector<FLGameObject *> update_handlers =
	std::vector<FLGameObject *>(DEFAULT_UPDATE_HANDLERS, nullptr);
...
int new_updator(FLGameObject *obj) {
	...
	if (obj == nullptr) {
		...
		// slots checked should not be needed, but I like to be sure we won't
		// have an infinite loop...
		while (update_handlers[next_free] != nullptr &&
			   slots_checked < NUM_UPDATE_HANDLERS) {
			next_free++;
			slots_checked++;
